...
class Sitemap
{
    ...
    public function generate($format = 'xml')
    {
        if ($this->isCached()) {
            ...
            ($format == 'sitemapindex') ? $this->cache->put($this->model->getCacheKey(), $this->model->getSitemaps(),
            ...
            case 'txt':
                return [
                    'content' => $this->view->make('sitemap::txt',
                        ['items' => $this->model->getItems()])->render(),
                    'headers' => ['Content-type' => 'text/plain'],
                ];
            case 'sitemapindex':
                return [
                    'content' => $this->view->make('sitemap::sitemapindex',
                        ['sitemaps' => $this->model->getSitemaps()])->render(),
